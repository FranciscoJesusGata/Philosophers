...
/*   Updated: 2021/12/08 16:53:50 by fgata-va         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_info *info)
{
	...
	while (i < info->number_of_philosophers)
	{
		...
		philosophers[i].left_fork = false;
		philosophers[i].right_fork = false;
		philosophers[i].left_hand = &forks[i];
		if (i == (info->number_of_philosophers - 1))
			philosophers[i].right_hand = &forks[0];
		else
			philosophers[i].right_hand = &forks[i + 1];
		i++;
	}
	return (philosophers);
}
